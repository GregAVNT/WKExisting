...
jQuery(document).ready(function ($) {
	...
	$('.cmb_upload_button').live('click', function () {
		...
		// Max added extra condition here:
		if(formfield.indexOf("[") != -1)
			buttonLabel = 'Use for ' + $('label[for=' + formfield.replace(/\[(.*)\]/g, "") + ']').text();
		else
    ...
    window.send_to_editor = function (html) {
		...
		if (formfield) {
			...
			if(formfield.indexOf("[") != -1) {
				formfield = formfield.replace(/\[(.*)\]\[(.*)\]\[(.*)\]/g, "_$1_$2_$3"); // added by Max
				$('#' + formfield.replace('_url', '_id')).val(itemid);
			} else {
